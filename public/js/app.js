...
const GRADE_META = {
  ...
  sgc10:  { label: 'SGC 10',  color: '#ff8c42' },
  sgc9:   { label: 'SGC 9',   color: '#e07030' },
};

function gradeKey(conditionOrGrade) {
  if (!conditionOrGrade) return 'raw';
  const m = conditionOrGrade.match(/^(PSA|BGS|CGC|SGC)\s+(.+)$/i);
  if (!m) return 'raw';
  ...
  try {
    ...
    if (gradeRowEl) {
      ...
      if (chips.length <= 1) {
        ...
        const otherLabels = chips.filter(c => c.key !== chips[0].key).map(c => c.label);
        const compareLabel = chips.length > 2 ? 'Compare all' : `Compare ${otherLabels.join(' & ')}`;
        gradeRowEl.innerHTML = chips.map(c => {
          const isActive = _cdActiveGrades.has(c.key);
          return `<button
            id="cdm-chip-${c.key}"
            class="cd-grade-chip${isActive ? ' active' : ''}"
            onclick="window.toggleCdmGradeChip('${c.key}')"
            style="--chip-color:${c.color};"
            title="Toggle ${c.label} price">${esc(c.label)}</button>`;
        }).join('') + `<button class="cd-grade-chip cd-chip-compare" onclick="window.compareAllCdmGrades()" title="Compare all grades on one chart" style="margin-left:auto;">⊞ ${esc(compareLabel)}</button>`;
      }
    }
...
function toggleCdmGradeChip(key) {
  ...
  document.querySelectorAll('.cd-grade-chip:not(.cd-chip-compare)').forEach(btn => {
    const k = btn.id.replace('cdm-chip-', '');
...
function buildMultiChartConfig(labels, datasets) {
  ...
  return {
    ...
    options: {
      responsive: true,
      plugins: {
        legend: { display: datasets.length > 1, labels: { color: '#aab0c4', font: { size: 11 } } },
        tooltip: {
          backgroundColor: '#13161f',
          borderColor: 'rgba(255,255,255,.07)',
          borderWidth: 1,
